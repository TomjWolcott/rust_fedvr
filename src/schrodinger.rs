...
use lapack::c64;
use lapack::fortran::{zgtsv, zpteqr, zsteqr};
use plotly::{Configuration, Layout, Mesh3D, Plot, Scatter, Scatter3D, Surface};
use plotly::color::Rgb as PlotlyRgb;
use plotly::common::{Line, Marker, Mode, Title};
use plotly::layout::Axis;
use plotly::mesh3d::Lighting;
use rayon::prelude::*;
...
fn test_compute_wave() {
    ...
    let _ = compute_wave_function(options);
}
...
pub trait DiffeqSystem {
    fn iter_matrix(&self) -> impl Iterator<Item=(usize, usize, Complex)> + Send + Sync;

    fn iter_constants(&self) -> impl Iterator<Item=(usize, Complex)> + Send + Sync;
}
...
pub struct TDSESystem {
    polys: LagrangePolynomials,
    xs: Vec<f64>,
    potential: Box<dyn Fn(f64) -> f64>,
    time_dependence: Box<dyn Fn(f64) -> f64>,
...
impl TDSESystem {
    pub fn new(
        ...
        let polys = LagrangePolynomials::new(n_t, nq_t, 0.0, t_final);
        let xs = (0..n_x).map(|n| n as f64 * box_size / (n_x - 1) as f64 - box_size / 2.0).collect();

        Self { polys, xs, potential, time_dependence }
    }
}
